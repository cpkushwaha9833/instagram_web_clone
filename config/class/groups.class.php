<?php
  class group{
    ...
    public function myGroups($id){
      ...
      if ($query->rowCount() == 0) {
        echo "<div class='home_last_mssg pro_last_mssg'><span>";
        if ($session == $id) {
    ...
    public function grpMembers($grp, $way, $limit){
      ...
      if ($count == 0) {
        if ($way == "direct") {
          echo "<div class='home_last_mssg pro_last_mssg'><span>No members</span></div>";
        }
      } else if ($count > 0) {
    ...
    public function getGrpPost($grp, $way, $count){
      ...
      if ($count == 0) {
        if ($way == "direct") {
          echo "<div class='home_last_mssg'><span>";
    ...
    public function getGrpPhotos($grp){
      ...
      $count = $query->rowCount();
      if ($count == 0) {
        echo "<div class='home_last_mssg pro_last_mssg'><span>Group has no photos</span></div>";
      } else if ($count > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          $post_id = $row->post_id;
          $userid = $row->user_id;
    ...
    public function getGrpVideos($grp){
      ...
      $count = $query->rowCount();
      if ($count == 0) {
        echo "<div class='home_last_mssg pro_last_mssg'><span>Group has no videos</span></div>";
      }
